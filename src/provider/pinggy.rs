...
const HOST: &str = "free.pinggy.io";
const PORT: u16 = 443;
const FREE_TTL_LIMIT: Duration = Duration::from_secs(60 * 60);

pub(super) fn free_ttl_limit() -> Duration {
    FREE_TTL_LIMIT
}
...
pub(super) async fn start(local_url: &str) -> Result<TunnelHandle> {
    ...
    let mut command = ssh::ssh_tunnel_command(true);
    command
        .arg("-p")
        .arg(PORT.to_string())
        .arg("-R")
...
pub(crate) async fn watch_pinggy_output<R>(
...
pub(crate) fn extract_pinggy_public_url(line: &str, regex: &Regex) -> Option<String> {
